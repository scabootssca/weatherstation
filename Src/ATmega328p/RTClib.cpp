...
uint8_t RTC_DS1307::isrunning(void) {
  I2c.read(uint8_t(DS1307_ADDRESS), uint8_t(0), uint8_t(1));
...
void RTC_DS1307::adjust(const DateTime& dt) {
  ...
  I2c.write(DS1307_ADDRESS, 0, toSend, sizeof(toSend));
  // Wire.beginTransmission(DS1307_ADDRESS);
...
Ds1307SqwPinMode RTC_DS1307::readSqwPinMode() {
  ...
  I2c.read(DS1307_ADDRESS, DS1307_CONTROL, 1);
  mode = I2c.receive();
...
void RTC_DS1307::writeSqwPinMode(Ds1307SqwPinMode mode) {
  I2c.write(DS1307_ADDRESS, DS1307_CONTROL, mode);
  // Wire.beginTransmission(DS1307_ADDRESS);
  // Wire._I2C_WRITE(DS1307_CONTROL);
...
void RTC_DS1307::readnvram(uint8_t* buf, uint8_t size, uint8_t address) {
  ...
  I2c.read((uint8_t)DS1307_ADDRESS, addrByte, size);
...
void RTC_DS1307::writenvram(uint8_t address, uint8_t* buf, uint8_t size) {
  int addrByte = DS1307_NVRAM + address;
  I2c.write(DS1307_ADDRESS, addrByte, buf, size);
  // int addrByte = DS1307_NVRAM + address;
  // Wire.beginTransmission(DS1307_ADDRESS);
  // Wire._I2C_WRITE(addrByte);
  // for (uint8_t pos = 0; pos < size; ++pos) {
